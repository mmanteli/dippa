...
def get_embed_for_a_vector(x, model, tokenizer):
    with torch.no_grad():
        output = model.base_model(**tokenizer(x, return_tensors='pt', padding=True))["last_hidden_state"].detach()
    model_output_list = [t.unsqueeze(0) for t in torch.unbind(output, dim=0)]
    embedded_pooled=[torch.mean(elem,axis=1).cpu() for elem in model_output_list]
    results=torch.vstack(embedded_pooled).numpy()
    return results
...
def cosine_sim(x,y,choose_k):
    ...
    for k in range(1, choose_k+1):
        ...
        for i in range(M.shape[0]): #M.shape[0] is the number of rows / input documents
            j=aligned[i,k] # index [1] onward for 2nd, 3rd... best match 
            # => index [0] gives the same words (the match perfectly).
            score=M[i,j]
...
def find_best_cosine_match(word, preds, choose_k, model, tokenizer):
    if word in preds:
        preds.remove(word)
    vector = [word]+preds
    emb=get_embed_for_a_vector(vector, model, tokenizer)
    k_sims = cosine_sim(emb, emb, choose_k)   # similarity with itself
    results = []
    for sims in k_sims:
